...
p1 <- fl %>%
  ...
  ggplot(aes(x=weekday, y=delay_rate)) +
    ...
    labs(title="Figure 2. Delay Rate by by Day of Week",x = "Day of Week", y="P(Delay ≥15min)")+
    scale_y_continuous(labels = scales::percent_format()) +
    theme_minimal()
```
...
p2 <- fl %>%
  ...
  ggplot(aes(x=DEP_TIME_BLK, y=delay_rate)) +
    ...
    labs(title="Figure 3. Delay Rate by Scheduled Departure Time Block", y="P(Delay ≥15min)") +
    scale_y_continuous(labels = scales::percent_format()) 
```
...
# Plot top features
xgb.ggplot.importance(importance_matrix = importance, top_n = 10, rel_to_first = TRUE) +
...
(p5 | p6) +
  plot_annotation(
    caption = "Figure 8. Model Accuracy by Hour (left) and by Carrier (right)",
